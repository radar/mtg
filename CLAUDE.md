...
## Trigger Queue (opt-in, rule 603.3b)

`Game.new(queue_triggers: true)` (default `false` — every existing spec/card is unaffected) makes triggered abilities go on the stack instead of running synchronously inside `notify!`. With it on, `Permanent#perform_trigger!` still evaluates `should_perform?`/`TriggeredAbility::OncePerTurn`-style bookkeeping immediately (via `TriggeredAbility#trigger!`, called once at trigger time), but defers the effect (`call`) by pushing the instance onto `Game#pending_triggers` instead of calling it right away.

Pending triggers are drained onto `game.stack` (via `TriggeredAbility#resolve!`, which just calls `call`) at the same checkpoints `state_based_actions_checkpoint!` already runs at (after every action, stack-item resolution, and choice resolution) — there is no real priority system yet (that's engine-roadmap workstream A2/A3), so this is the best available approximation of "the next time a player would receive priority." **Specs exercising this need to call `game.state_based_actions_checkpoint!` (or `game.check_state_based_actions!`) manually after a turn-step transition** (e.g. `current_turn.upkeep!`) — step transitions don't call it themselves, only `take_action`/stack/choice do.

Draining is APNAP order (rule 603.3b): `Game#players` is already active-player-first (see `next_active_player`'s `players.rotate(1)`), so the active player's triggers get put on the stack first (ending up lowest / resolving last) and the non-active player's go on after (ending up on top / resolving first) — no separate ordering calculation needed. A player with two or more simultaneously-pending triggers gets a `Choice::OrderTriggers` (one pick at a time, same shape as `Choice::LegendRule`); a lone trigger for a player skips the choice entirely via the existing `Stack#add_choice` single-choice auto-resolve. **A trigger resolving can itself create new pending triggers** (e.g. a token entering the battlefield firing another permanent's ETB trigger) — this correctly produces a new `Choice::OrderTriggers` mid-stack-resolution if that player now has 2+ pending, which pauses further stack resolution until it's resolved; don't assume one `stack.resolve!` call fully settles a queue-triggers game, loop until `stack.empty? && !stack.pending_choices?` if a spec needs a fully quiescent state (see `spec/game/integration/trigger_queue_spec.rb`'s `settle!` helper).
