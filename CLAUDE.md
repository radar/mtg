...
**"As ~ enters, choose a creature type"** (e.g. banners/tribal lands): No shared "choose type" infra beyond a bare `Magic::Choice::CreatureType < Magic::Choice` (mirrors `Magic::Choice::Color`, lives at `lib/magic/choice/creature_type.rb`). Add `attr_accessor :chosen_creature_type` on the card's class reopening, push a card-specific `CreatureTypeChoice` subclass (`resolve!(creature_type:)` sets `actor.card.chosen_creature_type = creature_type`) from an `EnterTheBattlefield` trigger. Later abilities read it via `source.card.chosen_creature_type` — this works because `permanent.card` is a unique Card instance per permanent, and static/activated abilities are instantiated with `source:` set to the permanent. Example: `PatchworkBanner`, `ThreeTreeCity`.
