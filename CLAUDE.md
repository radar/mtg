...
`printf 'Name {cost}\nType — Sub\nrules\nP/T\n' | bundle exec rake parse_card` writes `lib/magic/cards/<name>.rb` from plain card text (`Magic::CardParser` → `Magic::CardGenerator`). Header, type line and P/T live in `card_parser.rb`; each rules-text pattern is one file in `lib/magic/card_parser/rules/` (`Data.define` class that `include Rule`; found by glob, no registration). A rule implements `.parse(line)` (instance or nil) and, for nested classes, `hook` (`:static_abilities`/`:activated_abilities`/`:event_handlers`), `class_base_name`, `class_source(name)` (plus `handled_event` for event handlers). Add a mechanic = one rule file + `spec/card_parser/rules/<rule>_spec.rb`. Write rule classes as `class X < Data.define(...)`, not `X = Data.define do ... end` — constants in a `Data.define` block leak to the enclosing `Rules` module. Unrecognised rules text raises `UnsupportedCard`. Supported card kinds: creature (also Artifact/Enchantment Creature), instant, sorcery, enchantment, artifact (incl. legendary), Equipment (needs an `Equip` line), Aura (needs an `Enchant` line), Saga (needs chapter lines), land and basic land; other type lines/subtypes raise `UnsupportedCard`. A rule can also supply `body_source` (raw Ruby for the class reopening, e.g. `enchant "Creature"`) or `dsl_lines` (lines for the DSL block, e.g. `equip [...]`). Parenthesised reminder text is stripped before parsing, and the card's own name in rules text is replaced with `~`. One-sentence game effects (deal damage, draw, gain life, destroy target) are separate reusable classes in `lib/magic/card_parser/effects/` (`include Effect`; `.parse(text)`, `target_choices`, `resolve_call`); `Rules::SpellEffect` merges an instant's or sorcery's effect lines (it must have at least one) into `target_choices`/`resolve!`, in order; at most one may be targeted. An effect that is a player choice (`Effects::Scry`) defines `choice_base`/`choice_class_name`: the generator adds a `Choice` subclass, and the effects after it run in that subclass's `resolve!` once the player has chosen (Opt is generated this way). One choice per spell, and nothing targeted after it. Reuse `Effect.parse` for future trigger/activated-ability rules. An effect that needs a constant (a token class from `Effects::CreateToken`) returns it from `definitions`; the wrapping rule emits it next to the code that uses it. `resolve_call` runs with `self` = the spell or chapter ability, so an effect that asks the player to choose passes `actor: self` to its Choice (`Effects::CopyTokens` → `Magic::Choice::CopyTokens`). Saga chapters (`I — ...`, `II, III — ...`) are `Rules::Chapter`, which `merge`s every chapter line into one rule emitting `ChapterN < Saga::ChapterAbility` classes and `def chapters`; chapters must be I, II, III... without gaps. A targeted chapter effect becomes a nested `Choice < Magic::Choice::Targeted` added on resolution; a choice effect (scry) gets its `choice_class_name` subclass inside the chapter. `Saga::ChapterAbility` has `game`, `controller`, `battlefield` and `trigger_effect` (delegating to the saga permanent), so effect code reads the same in a chapter as in a spell. `bundle exec rake parse_card` forces stdin to UTF-8 so the `—` in type lines and chapters parses under a C locale.
