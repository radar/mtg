...
**Calling `permanent.add_counter(...)` directly from card code silently skips `DoublingSeason`/other counter-doubling replacement effects** — that raw method (`lib/magic/permanent.rb`) is the low-level mutation `Effects::AddCounterToPermanent#resolve!` itself calls; it never goes through `game.add_effect`, so `ReplacementEffectResolver` never sees it. Any card putting counters on a permanent must go through `trigger_effect(:add_counter, counter_type:, target:, amount: 1)` (or the `add_counter` shorthand above) instead, even for "put a counter on each creature you control"-style abilities where it's tempting to just loop and call the permanent method. Bug found via `Aron, Benalia's Ruin`'s activated ability; combo test: `spec/game/integration/aron_benalias_ruin_doubling_season_spec.rb`.
