...
RSpec.describe Magic::Cards::EclipsedMerrow do
  ...
  let!(:creature) { ResolvePermanent("Eclipsed Merrow", owner: p1) }
  let(:choice) { game.choices.last }

  it "is a 2/3 Merfolk Scout" do
    expect(creature.power).to eq(2)
    expect(creature.toughness).to eq(3)
    expect(creature.type?("Merfolk")).to eq(true)
    expect(creature.type?("Scout")).to eq(true)
  end

  it "looks at the top four cards and offers only Plains, Island cards" do
    expect(choice).to be_a(Magic::Choice::LookAtTopCards)
    expect(choice.looked_at.map(&:name)).to eq(%w[Swamp Plains Forest Island])
    expect(choice.choices.map(&:name)).to eq(%w[Plains Island])
  end
  ...
  it "puts the chosen card into your hand and the rest on the bottom of the library" do
    ...
    expect(p1.library.last(3).map(&:name)).to contain_exactly("Swamp", "Forest", "Island")
  end
  ...
  it "may take no card, putting all four on the bottom" do
    ...
    expect(p1.library.last(4).map(&:name)).to contain_exactly("Swamp", "Plains", "Forest", "Island")
  end
end
