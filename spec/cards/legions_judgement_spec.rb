...
RSpec.describe Magic::Cards::LegionsJudgement do
  ...
  let(:card) { Card("Legion's Judgement") }

  context "with a creature with power over 4" do
    let!(:hill_giant_herdgorger) { ResolvePermanent("Hill Giant Herdgorger", game: game, owner: p2) }
  ...
  context "with a creature with power under 4" do
    let(:wood_elves) { ResolvePermanent("Wood Elves", game: game, owner: p2) }
